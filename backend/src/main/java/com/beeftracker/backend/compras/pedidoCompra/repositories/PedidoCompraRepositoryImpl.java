...
public class PedidoCompraRepositoryImpl implements PedidoCompraRepository {
    ...
    public void salvar(PedidoCompraData data) {
        String sql = "INSERT INTO pedido_compra (fornecedor_id, valor_total, status, observacao) " +
                "VALUES (:fornecedorId, :valorTotal, :status, :observacao)";

        jdbcTemplate.update(sql, new MapSqlParameterSource()
                .addValue("fornecedorId", data.fornecedorId())
                .addValue("valorTotal", data.valorTotal())
                .addValue("status", data.status() != null ? data.status() : "PENDENTE")
    ...
    public List<PedidoCompra> pesquisar(Long fornecedorId, String status, int page) {
        ...
        sql.append("LIMIT 10 OFFSET ").append(offset);
    ...
    private PedidoCompra mapRow(ResultSet rs) throws SQLException {
        ...
        Metadata metadata = new Metadata(
                rs.getObject("criado_em", OffsetDateTime.class) != null
                        ? rs.getObject("criado_em", OffsetDateTime.class).toLocalDate()
                        : null,
                rs.getObject("atualizado_em", OffsetDateTime.class) != null
                        ? rs.getObject("atualizado_em", OffsetDateTime.class).toLocalDate()
                        : null,
                rs.getLong("id"),
                rs.getString("token"));
    ...
    @Override
    public PedidoCompra findByViagem(Long viagemId) throws ResourceNotFoundException {
        String sql = "SELECT p.id, p.token, p.viagem_id, p.fornecedor_id, p.valor_total, p.status, " +
                "p.observacao, p.data_emissao, p.data_entrega, p.criado_em, p.atualizado_em " +
                "FROM pedido_compra p WHERE p.viagem_id = :viagemId LIMIT 1";

        List<PedidoCompra> resultado = jdbcTemplate.query(sql,
                new MapSqlParameterSource().addValue("viagemId", viagemId),
                (rs, rowNum) -> mapRow(rs));
