...
const char* mqttPass = "31xm30JsNbKCn6KoKNnE";
...
// Protótipos de funções para garantir compilação correta
...
bool tryConnectMQTT() {
  ...
  if (mqtt.connect("BeefTrackerESP32", mqttUser, mqttPass)) {
    mqttOk = true;
...
void flushOffline() {
  ...
  while (f.available()) {
    ...
    if (mqtt.publish("sensor/leitura", line.c_str())) {
      sent++;
    } else {
...
void setup() {
  ...
  if (wifiOk) {
    ...
    while (time(nullptr) < 1000000000 && millis() - startNtp < 10000) {
      delay(500);
      Serial.print(".");
    }
  ...
  // --- SEU AJUSTE SOLICITADO NO SETUP ---
  if (mqttOk) {
    tft.setTextColor(TFT_GREEN, TFT_BLACK);
...
void loop() {
  // Mantém a conexão ativa e checa status
  checkWiFi();
  checkMQTT();
  ...
  if (isnan(temp) || isnan(hum)) {
    ...
    delay(2000);
    return; // Reinicia o loop se houver falha na leitura do sensor
  }
...
void drawDashboard(float t, float h) {
  ...
  tft.fillCircle(140, 165, 8, mqttOk ? TFT_GREEN : tft.color565(30, 60, 30));
  tft.setTextColor(TFT_WHITE, TFT_BLACK);
  tft.setTextDatum(ML_DATUM);
  tft.drawString("OK", 152, 165, 1);

  tft.fillCircle(140, 187, 8, (t > 30.0) ? TFT_RED : tft.color565(60, 20, 20));
...
void drawAlert(float temp) {
  ...
  tft.setTextColor(tft.color565(0, 200, 255), TFT_BLACK);
  if (gpsFixed) {
    tft.drawString(String(gpsLat, 4) + ", " + String(gpsLng, 4), 160, 205, 1);
  } else {
